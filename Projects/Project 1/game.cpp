...
void saveState(GameState *);
bool loadState(GameState *);
...
int main()
{
    ...
    GameState *state; 
    state = new GameState();
    bool loaded = loadState(state);
    //bool loaded = false; // Debug
    if (!loaded)
    {
...
bool loadState(GameState *gState)
{
    std::fstream ifile;
    ifile.open("game.bin", std::fstream::in);
    if (ifile)
    {
        std::cout << "game file exists, loading" << std::endl;
       
        ifile.read(reinterpret_cast<char *>(gState), sizeof(GameState));
        //ifile.close();
        return true;
    }
    else
...
void saveState(GameState *gState){
    std::fstream ifile;
    ifile.open("game.bin",  std::fstream::in |  std::fstream::out |  std::fstream::trunc);
    if (ifile)
    {
        std::cout << "> Saved Game";
        ifile.write(reinterpret_cast<char *>(gState), sizeof(GameState));
...
void genBoard(GameState *state)
{
    ...
    for (int y = 0; y < 10; y++)
    {
        for (int x = 0; x < 10; x++)
        {
            state->plyStates[0].board[y][x] = Tile::EMPTY;
            state->plyStates[0].hitBrd[y][x] = Tile::EMPTY;

            state->plyStates[1].board[y][x] = Tile::EMPTY;
            state->plyStates[1].hitBrd[y][x] = Tile::EMPTY;
        }
    }
