...
int main(int argc, char** argv) {
    ...
    Test *test2=rdRecBn(binFile,12);
    //Test out your writing to a Binary File Here
    std::cout << "Original Record #12" <<std::endl;
    prntTxt(test2);
    test2->s *= 10;
    std::cout << "Modified Record #12" <<std::endl;
    prntTxt(test2);
    wrtRcBn(binFile, test2, 12);
    delete test2;
    
    test2=rdRecBn(binFile,12);
    std::cout << "Loaded Record #12" <<std::endl;
    prntTxt(test2);
    //Close the files
    txtFile.close();
    binFile.close();
...
void wrtRcBn(fstream &out,Test *data,int record){
    long cursor = 0L;

    cursor = record * sizeof(data);

    out.seekg(cursor, ios::beg);
